...
.venv/bin/python -m flask --app app init-db  # 11 tables per utils/db.py:5 SCHEMA
...
## Architecture
- 4 blueprints in `app.py:76-84`: `routes/public.py` (`/`, `/map`, `/centers`, `/centers/<id>`, `/weather`, `/hotlines`), `routes/auth.py` (`/admin/login` canonical + legacy obscure alias on the same view — path is public by design per GH #3, never rely on its secrecy; bad creds → uniform 401, locked → 429 generic; logout is POST-only `/admin/logout`), `routes/admin.py` (`/admin/*` guarded by `utils/security.py:4` `login_required` → redirect to login), `routes/api.py` (`/api/centers`, `/api/centers/version`, `/api/centers/<id>`, `/api/hotlines`, `/api/ncr-lgus` static 17 LGUs, `/api/weather`, `/api/air-quality`, `/api/environment`, `/api/groups`, `/api/locations`, `/api/groups/<code>/locations`, `/api/announcements`, `/api/earthquakes`, `/api/fires`). API blueprint is CSRF-exempt `app.py:88`.
- `create_app()` self-heals file DBs via `init_db()` on startup (`app.py:54-59`; skipped for `:memory:`) — new tables appear without wiping data. `run_gui.py:93-113` does the same + `seed_db()`; binds `127.0.0.1:5000` by default (`LIGTASPH_HOST=0.0.0.0` for LAN demos) with `use_reloader=False` so the browser opens once.
...
- Sprint 2 import (`scripts/import_evac_centers.py`, pure functions + stats dict): `data/ncr_evacuation_centers.geojson` (868 features → 836 live + 32 staged). City names normalized via `CITY_MAP` (unknown → quarantine, never guessed); addresses synthesized `<name>, <barangay>, <city>` with deterministic suffixes (`plan_addresses()` — re-imports UPDATE, never duplicate); re-import refreshes geocoded fields only, never clobbers admin-set capacity/occupancy/contacts. GeoJSON coords are `[lon, lat]` — the importer asserts ranges. GH #7: the file is sha256-pinned (`app_meta.geojson.sha256`, recorded by boot/CLI, surfaced via `X-Dataset-*` headers); a changed hash is REFUSED at boot (nothing ingested, hash kept as `geojson.pending_sha256`, served old rows) — approve explicitly via admin dataset banner/POST or `import-geojson` CLI, which records the hash and clears pending. Fresh DBs (no recorded hash) still auto-import once. **Data-file PRs still need human review; the gate enforces the review step.** Boot auto-import runs in a daemon thread (serving never waits) behind a best-effort file lock (`utils/import_lock.py`, fail-open); `/api/centers` defaults to 50 rows — frontend fetches pass explicit `?limit=1000`.
