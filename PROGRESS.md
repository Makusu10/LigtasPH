...
- **Tests**: 247 pytest (incl. `tests/test_api_standardization.py`: error-envelope codes, pagination headers/envelope, idempotency replay/conflict, location UPSERT), plus settings/status/keys/banner/history/timezone suites. Windows note: suite assumes UTF-8 file reads (`encoding="utf-8"` fixed in `test_noah.py`).

## Recent milestones (newest first)
- **Community rain reports** — anonymous ground truth on the weather tab (“Is it raining in your area?”: none/light/heavy, flooding follow-up only on heavy). `rain_reports` table (no identity, 3h expiry window, 24h prune), `POST /api/rain-reports` (validated, 10/min) + aggregate-only `GET` (counts, never rows; 60/min); location context from explicit GPS/LGU/city only, never the default grid; OpenAPI contract updated; privacy policy discloses the new row. `tests/test_rain_reports.py` (6 tests). 241 → 247 passing.
